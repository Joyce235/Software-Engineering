...
class DuckSimView(private val duckPond: DuckPond) : JPanel() {
    ...
    val rectangleList = listOf(
        ...
        Rectangle(150, 450, 100, 100), // rectangle for duck 4
        Rectangle(275, 450, 100, 100), // rectangle for duck 5
        Rectangle(400, 450, 100, 100), // rectangle for duck 6
        Rectangle(525, 450, 100, 100)  // rectangle for duck 7
    )
    ...
    private fun paintQuack(g: Graphics) {
        ...
        g.color = Color.BLACK
        val quack: String = duckPond.currentDuck!!.quackText
    ...
    private fun paintDuck(g: Graphics, duck: Duck, rect: Rectangle) {
        ...
        if (duck.isOnDSWC && duck.isFree && duck.state === State.WELCOMING) {
            drawCenteredTextInRectangle(g, text = "Welcome!", rectangle = rect, yOffset = 35, color = Color.BLUE)
            duck.swim()
        } else if (duck.isOnDSWC && duck.state === State.WELCOMING) {
            drawCenteredTextInRectangle(g, text = "Beware!", rectangle = rect, yOffset = 35, color = Color.BLUE)
            duck.swim()
        }

        // If the duck is in the welcoming committee, put a little W in the bottom left corner of the square.
        if (duck.isOnDSWC) {
            drawCenteredTextInRectangle(
                g,
                text = "W",
                rectangle = rect,
                xOffset = -28,
    ...
    private fun drawCenteredTextInRectangle(
        ...
        val xPos =
            rectangle.x + rectangle.width / 2 - textWidth / 2 + xOffset // centered (positive offset move it right)
